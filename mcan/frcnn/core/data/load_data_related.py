...
from core.scene_graph.gqa_matching import GQADataset
...
class DataSet(Data.Dataset):
        ...
        if self.__C.IS_GQA :
            ...
            for split in split_list:
                ...
                if split in ['val'] :
                    ...
                    self.iid_to_img_feat = GQA.extractembeddings_related(self.list_images, self.iid_to_img_feat)
    ...
    ##            img_feat = np.load(self.iid_to_img_feat_path[str(ques['image_id'])])
    ##            img_feat_x = img_feat['x'].transpose((1, 0))
    ##        print (type(img_feat_x))
    ##        print (img_feat_x.shape)
    ##        img_feat_iter = proc_img_feat(img_feat_x, self.__C.IMG_FEAT_PAD_SIZE)
