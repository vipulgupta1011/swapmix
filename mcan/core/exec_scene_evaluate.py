...
from core.data.load_data_scene_evaluate import DataSet
from core.model.net_scene import Net
    ...
    def eval(self, dataset, state_dict=None, valid=False):
       ...
       #     ), end='          ')
        ...
        for k in range(num_images) :
            ...
            '''
            img_feat_obj_iter_mapping -> list of features with object perturbations
            img_feat_attr_iter_mapping -> list of features with attributes corresponding to object perturbations to make the swapped faetures real
            changes_mapping -> dict of changes corresponding to each object
            irrelevant_questions_mapping -> questions for which object of interest is irrelevant
            '''
            img_feat_obj_iter_mapping, img_feat_attr_iter_mapping, ques_dict, ans_dict, img_feat_bbox_mapping, changes_mapping, irrelevant_questions_mapping, img = dataset.extract(k)
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

            result[img] = {}
            ans_map = {}
            for ans in ans_dict :
                ans_map[ans] = np.argmax(ans_dict[ans])

            result[img]['answers'] = ans_map
            for obj in img_feat_obj_iter_mapping :
                #pdb.set_trace()
                result[img][obj] = {}
                result[img][obj]['changes'] = changes_mapping[obj]

                img_feat_obj_iters = img_feat_obj_iter_mapping[obj]
                img_feat_attr_iters = img_feat_attr_iter_mapping[obj]

                c = 0
                ques_dict_obj = {}
                ques_dict_obj = { relevant_key: ques_dict[relevant_key]  for relevant_key in irrelevant_questions_mapping[obj]}
                num_ques = len(ques_dict_obj)
                result[img][obj]['relevant_questions'] = irrelevant_questions_mapping[obj]
                
                if num_ques == 0:
                    continue

                result[img][obj]["prediction"] = {}
                for q in range(len(img_feat_obj_iters)) :
                    c +=1

                    img_feat_obj_iter = img_feat_obj_iters[q]
                    img_feat_attr_iter = img_feat_attr_iters[q]

                    img_feat_obj_iter = torch.tensor(img_feat_obj_iter, dtype=torch.float32)
                    img_feat_attr_iter = torch.tensor(img_feat_attr_iter, dtype=torch.float32)
                    img_feat_obj_iter = torch.unsqueeze(img_feat_obj_iter,0)
                    img_feat_obj_iter = torch.cat(num_ques * [img_feat_obj_iter])
                    img_feat_attr_iter = torch.unsqueeze(img_feat_attr_iter,0)
                    img_feat_attr_iter = torch.cat(num_ques * [img_feat_attr_iter])
                    ...
                    for i in range(n_batches) :
                        ...
                        result[img][obj]["prediction"][q]["pred"].append(pred_argmax)
