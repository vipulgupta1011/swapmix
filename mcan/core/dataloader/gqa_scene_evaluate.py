...
class GQADataset() :
    def __init__(self, scenegraphs_json, vocab_json, embedding_json, obj_matching, attr_matching, ques_list, object_mapping, no_of_changes, allow_random=False) :
    ...
    def scene2embedding_attr(self, imageid, ele) :
        #print (imageid)
        meta = dict()
        embeds = dict()
        scenegraphs_json = self.scenegraphs_json
        ...
        for obj_number in info_raw['objects'] :
            ...
            if obj_number == ele :
                ...
                boxes[obj_number] = box
                count = 0
                ## Changing only the attributes and making a list of attribute features
                ## object list will have same object feature for all entries
                for attr in obj['attributes'] :
                    try :
                        attrs_match = self.attr_matching[attr]
                    except :
                        continue
                    ## replace the selected attribute with a similar attribute
                    for attr_match in attrs_match :
                        temp_attr = []
                        try :
                            for attr_nested in obj['attributes'] :
                                if attr_nested == attr :
                                    cid = self.getattr2idx(attr_match)
                                else :
                                    cid = self.getattr2idx(attr)
                                attr_embed = self.getembedding(cid)
                                temp_attr.append(attr_embed)
                        ...
                        embeds[ele]['obj_embed'].append(label_embed)
                        change_text = str(name) + '_' +str(attr) + 'to' + str(attr_match)
                        changes.append(change_text)
    ...
    def extractembeddings_attr(self,images_list, mapping) :
        final_embeddings = mapping
        images = images_list
        i=0
        print ('GQA embedding extraction in place')
        for image in images :
            ...
            for obj in objects_raw :
                irrelevant_ques = self.get_irrelevant_ques(image, obj)
                embeddings, bboxes, changes = self.scene2embedding_attr(image, obj)
                #embeddings = embeddings.astype(np.double)
                final_embeddings[image][obj] = {}
                final_embeddings[image][obj]['objandattr'] = embeddings
                final_embeddings[image][obj]['bboxes'] = bboxes
