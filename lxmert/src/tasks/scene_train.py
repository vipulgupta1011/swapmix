...
from tasks.scene_model import GQAModel
from tasks.dataloader.scene_data import GQADataset, GQATorchDataset, GQAEvaluator, GQATorchDataset_swapmix, GQATorchDataset_valid 
import pdb
...
### To be used only for SwapMix validation set during training
def get_tuple_valid(splits: str, bs:int, shuffle=False, drop_last=False) -> DataTuple:
    dset = GQADataset(splits)
    tset = GQATorchDataset_valid(dset)
    evaluator = GQAEvaluator(dset)
    data_loader = DataLoader(
        tset, batch_size=bs,
        shuffle=shuffle, num_workers=args.num_workers,
...
class GQA:
    def __init__(self):
        ...
        if args.valid != "":
            ...
            if args.SwapMix :
                pdb.set_trace()
                self.valid_tuple = get_tuple_valid(
