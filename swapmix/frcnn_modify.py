...
class FRCNN() : 
    def __init__(self, __C) :
        ...
        ## File containing basic information for each image : width, height, location in the h5 files
        self.info_file = json.load(open(__C.FASTRCNN_INFO_FILE))

        self.val_scene = json.load(open(__C.VAL_SCENE_GRAPH))

        ## File containing information for objects similar to object of interest based on Glove embeddings similarity
        self.obj_matching = json.load(open(__C.OBJ_MATCHING))

        ## File containing information for location of a particular object in various images in the dataset
    ...
    ## This function is used to generate visual context perturbations for the input image
    ''' Input - image number, unrelevant objects info
